    ...
    public class SqlQuery
    {
            ...
            Action<IDataReader> action)
        {
            DataAccess.Execute(this, () =>
            {
                using (DbDataReader dr = Command.ExecuteReader())
                {
                    while (dr.Read() && maxRecordsToFetch-- >= 0) action(dr);
                }
            },
            connection: this.Connection);
        }
        ...
        public List<T> ExecuteBuildEntities<T>(Func<IDataRecord, T> entityBuilder, Boolean returnNullListOnError)
        {
            ...
            DataAccess.Execute(this, () =>
            {
                try
                {
                    using (DbDataReader dr = Command.ExecuteReader())
                    {
                        while (dr.Read()) retvalue.Add(entityBuilder(dr));
                    }
                }
                catch (Exception ex)
                {
        ...
        {
            Int32 result = 0;
            DataAccess.Execute(this, () => result = Command.ExecuteNonQuery(), Connection);
            return result;
        }
        ...
        {
            if (value != nullValue) SetParam(parameterName, value, DbType.Int32);
            return this;
        }
