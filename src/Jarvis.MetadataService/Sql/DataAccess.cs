...
namespace Jarvis.MetadataService.Sql
{
    public static class DataAccess
    {
        static ILog logger = LogManager.GetLogger(typeof(DataAccess));
        ...
        public static void Execute(SqlQuery q, Action executionCore, ConnectionStringSettings connection)
        {
            using (DataAccess.ConnectionData connectionData = DataAccess.CreateConnection(connection))
            {
                try
                {
                    using (q.Command)
                    {
                        ...
                        if (logger.IsDebugEnabled) logger.Debug(DumpCommand(q.Command));
                        executionCore();
                ...
                {
                    if (logger.IsErrorEnabled) logger.Error("Could not execute Query:" + DumpCommand(q.Command), ex);
        ...
        public static String DumpCommand(DbCommand command)
        {
            ...
            sb.AppendLine("--Reputation Manager Data Access Dumper:");
        ...
        public static void Execute(Action<DbCommand, DbProviderFactory> functionToExecute, ConnectionStringSettings connection)
        {
            ...
            using (ConnectionData connectionData = CreateConnection(connection))
            {
                ...
                {
                    if (logger.IsErrorEnabled) logger.Error("Could not execute Query:" + DumpCommand(command), ex);
                    connectionData.Rollback();
                    throw;
                }
            }
